...
app.post('/api/workflows', async (req, res) => {
  ...
  try {
    const workflow = new Model({ id, nodes });
    await workflow.save();
...
app.post('/api/upload', upload.single('file'), async (req, res) => {
  ...
  if (!nodeTypes || !Array.isArray(nodeTypes) || !workflow || !Array.isArray(workflow.nodes)) {
    return res.status(400).json({ error: 'Invalid node types or workflow structure' });
  }
...
const sendPostRequest = async (data, url) => {
  ...
  try {
    ...
    throw new Error('Invalid JSON data. Make sure to convert Data to JSON before making Post Request');
  }
